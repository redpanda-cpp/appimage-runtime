...
if [ -z "${ALPINE_ARCH}" ]; then
    ...
    echo "Example values: x86_64 x86 armhf aarch64 riscv64"
    exit 2
fi
...
wget "http://dl-cdn.alpinelinux.org/alpine/v3.20/releases/${ALPINE_ARCH}/alpine-minirootfs-3.20.3-${ALPINE_ARCH}.tar.gz"
mkdir -p ./miniroot
cd ./miniroot
...
elif [ "$ALPINE_ARCH" = "riscv64" ] ; then
    echo "Architecture is riscv64, hence using qemu-riscv64-static"
    sudo cp "$(which qemu-riscv64-static)" miniroot/usr/bin
    sudo chroot miniroot qemu-riscv64-static /bin/sh -ex /scripts/chroot/build.sh
else
